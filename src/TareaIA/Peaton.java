...
public class Peaton extends TimerTask implements Constantes {
    ...
    public void moverPeaton() {
        if (peaton.X==p2.x && peaton.Y<p3.y && peaton.Y>=p2.y)     moverCeldaAbajo();
        else if (peaton.X>=p1.x && peaton.Y==p1.y && peaton.X<p2.x) moverCeldaDerecha();
        else if (peaton.X<=p3.x && peaton.Y==p3.y && peaton.X>p4.x) moverCeldaIzquierda();
        else if (peaton.X==p4.x && peaton.Y<=p4.y && peaton.Y>=p1.y) moverCeldaArriba();   
    }
